...
flowchart LR
  ...
  ES --> DB[(MySQL)]
  O --> DB
  subgraph L["Arrow legend"]
    LD["Dependency"] -->|depends on| LP["Dependency target"]
    LA["Adapter"] -. implements .-> LPort["Port"]
  end
```

图例中的实线箭头是依赖箭头，始终从依赖者指向依赖目标；虚线 `implements` 箭头是 realization，始终从 adapter 指向 port，不表示 `suno-core` 依赖 Operations。允许的依赖箭头只有 feature 模块到 `suno-core`，bootstrap 到所有运行时 feature 模块的公开 `api`，以及 POM 已声明的受控公开依赖：Marketplace → Recycle、Payment → Marketplace、Operations → Identity/Recycle/Marketplace/Payment。禁止 feature 依赖 bootstrap、另一个模块的 `internal`/persistence 包，或直接构造其实体。bootstrap 是唯一 composition root，负责 Spring wiring、配置和边界 adapter；它不拥有业务规则。
